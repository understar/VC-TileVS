...
class tiledMatrix {
	public:
		...
		int get_time_of_day(struct timeval *tp, void *tzp)
		{
			time_t clock;
			struct tm tm;
			SYSTEMTIME wtm;
...
void tiledMatrix<T>::loadTile2(int i,int j) {
   ...
   /* gettimeofday(&end, NULL);
    ...
    gettimeofday(&start, NULL);		*/
		...
		LZ4_decompress_fast (bufferTempCompressao, reinterpret_cast<char *>(matrizes[id]) , linhasTiles*colunasTiles*sizeof(T) );
...
void tiledMatrix<T>::removeTileDaMemoria(int i,int j) {
  ...
  /*  long tempoComprimir, tempoGravar, seconds, useconds;    
    ...
    struct timeval start, end;
    gettimeofday(&start, NULL);*/
		bytesCompressaoBloco[i][j] = LZ4_compress_default(reinterpret_cast<char *>(matrizes[id]), bufferTempCompressao, 
			linhasTiles*colunasTiles*sizeof(T), LZ4_compressBound(linhasTiles*colunasTiles*sizeof(T)) );
 /*   gettimeofday(&end, NULL);
